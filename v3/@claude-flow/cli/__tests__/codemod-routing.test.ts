/**
 * Route-time dry-run gating for the executable Tier-1 codemod path.
 *
 * ADR-0322 (Batch-U deferred fix B, hand-port of upstream 0988d92ce / ADR-143):
 * the enhanced router now runs a route-time dry-run so Tier-1 (`handler: 'codemod'`,
 * $0, no LLM) only fires when the deterministic codemod would actually change the
 * target file; a verified no-op falls through to model routing. Non-deterministic
 * intents never claim Tier-1.
 */
...
describe('route-time dry-run (ADR-0322 / ADR-143)', () => {
