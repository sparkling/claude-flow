/**
 * Deterministic codemod engine tests (ADR-0322 / ADR-143).
 *
 * ADR-0322 (Batch-U deferred fix B, hand-port of upstream 0988d92ce / ADR-143):
 * pins the executable Tier-1 contract — the three deterministic codemods really
 * transform sample source (var→const, remove-console strips console.*,
 * add-logging inserts an entry log) and a no-op case (e.g. remove-console on
 * console-free code) is correctly reported as no-change. Pure TS-compiler engine
 * (no embeddings / ONNX / RVF), so no ADR-0068 mpnet conflict.
 */
