  ...
  try {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // #8 prompt caching (upstream c983c0d80, hermes-agent pattern): mark the
        // (often large, stable) system prompt as an ephemeral cache breakpoint so
        // repeated agent_execute calls with the same system prompt hit Anthropic's
        // prompt cache (~90% discount on cached input tokens, 5-min TTL). Serves
        // the fork's cost-tracking posture (ADR-0307).
        ...(input.systemPrompt
