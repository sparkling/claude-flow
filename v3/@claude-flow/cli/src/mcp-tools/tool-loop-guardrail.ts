 ...
 *
 * Fork port of upstream c983c0d80 #6 — ADR-0327 (Batch-U follow-up). Detects an
 * agent stuck repeating the same failing command — a real silent failure mode
 * where a loop burns turns with no signal. This is ORTHOGONAL to the security
 * `tool-output-guardrail` (which detects indirect prompt injection in tool
 * *output*); this one watches the agent's own *call* pattern.
 *
