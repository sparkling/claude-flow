/**
 * Parent-death watchdog (#2234, ADR-0320 — Batch-U hand-port of upstream 455152da0).
 *
 ...
 * hook and exit cleanly. The stdin-EOF handler (bin/cli.js) closes the common
 * case; this catches the reparent-without-EOF tail (the inline MCP path has no
 * SIGTERM/SIGINT fallback).
 *
 * FORK WIRING NOTE (ADR-0320): upstream wired this into `mcp.ts` startCommand,
 * but the fork's live `ruflo mcp start` path is `bin/cli.js` inline mode (the
 * wrapper proxies to bin/cli.js, which handles MCP inline and short-circuits
 * before the CLI dispatcher reaches mcp.ts). So the fork installs this from
 * `bin/cli.js`, NOT mcp.ts — wiring it into mcp.ts would be dead code here.
 *
