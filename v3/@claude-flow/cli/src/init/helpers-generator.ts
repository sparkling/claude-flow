...
export function generateHookHandler(): string {
  ...
  const lines = [
    ...
    ' */',
    '',
    "import { existsSync, statSync, mkdirSync, openSync, closeSync } from 'fs';",
    ...
    '      try {',
    '        var ti = hookInput.tool_input || hookInput.toolInput || {};',
    '        var captureArgs = [',
    "          'hooks', 'post-task',",
    ...
    "        var logFd = 'ignore';",
    '        try {',
    "          var metricsDir = join('.claude-flow', 'metrics');",
    '          if (!existsSync(metricsDir)) mkdirSync(metricsDir, { recursive: true });',
    "          logFd = openSync(join(metricsDir, 'post-task-capture.log'), 'a');",
    "        } catch (e) { logFd = 'ignore'; }",
    "        var captureCmd = process.env.RUFLO_TASK_CAPTURE_CMD || '';",
    '        var child = captureCmd',
    ...
    "        child.on('error', function () { /* best-effort capture; never fail the hook */ });",
    '        child.unref();',
    "        if (typeof logFd === 'number') { try { closeSync(logFd); } catch (e) { /* child holds its dup */ } }",
    "        console.log('[ADR-0290] episode capture dispatched (agent=' + (ti.subagent_type || 'unknown') + ')');",
    '      } catch (e) { console.error(`[FAIL] hook-handler.post-task.capture: ${e?.message || e}`); }',
    '    }',
