/**
 * Inbound message dispatcher (ADR-109 upstream; ported to the fork as
 * ADR-0309 T1′).
 *
 ...
 *
 * ADR-0309 adaptation: `AgentMessage` is imported from the fork's
 * transport re-export (`../transport/agentic-flow-loader.js`) rather
 * than the bare `agentic-flow/transport/loader` specifier upstream
 * uses. The fork makes `agentic-flow` an optional dep (ADR-0297 R2)
 * and funnels the type surface through that loader; routing through it
 * keeps this module resolvable when `agentic-flow` is absent. Type-only
 * import — erased at compile, runtime-identical.
 */
