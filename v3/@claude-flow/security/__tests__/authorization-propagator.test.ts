/**
 * Tests for AgentAuthorizationPropagator
 * (ADR-0324 / upstream 1c98cbee6 ADR-144 P1, ruvnet/ruflo#2248).
 *
 ...
 *  - SELF-INERT: checkToolCallOrInert with no scope is a no-op `allow`
 *    (identical to pre-feature behaviour — nothing to enforce).
 *  - SELF-INERT: escape hatch (RUFLO_STRICT_AUTH=false) forces pass-through
 *    even when a denying scope is present.
 *  - ON PATH: with a real scope present and enforcement on, a denying scope
 *    actually rejects.
 */
...
describe('checkToolCall — typed decisions (raw enforcement)', () => {
  const prop = new AgentAuthorizationPropagator();
...
describe('checkToolCallOrInert — ON + self-inert (ADR-0324)', () => {
  const prop = new AgentAuthorizationPropagator();

  afterEach(() => {
    delete process.env.RUFLO_STRICT_AUTH;
    delete process.env.CLAUDE_FLOW_STRICT_AUTH;
  });

  // SELF-INERT: the precondition (a scope) is absent → no behaviour change.
  it('no scope → allow (self-inert-no-scope), identical to pre-feature behaviour', () => {
    expect(prop.checkToolCallOrInert('shell_exec', undefined)).toEqual({
      allowed: true,
      reason: 'self-inert-no-scope',
    });
    expect(prop.checkToolCallOrInert('shell_exec', null)).toEqual({
      allowed: true,
      reason: 'self-inert-no-scope',
    });
  });

  // SELF-INERT: a tool that WOULD be denied by the scope is allowed because
  // enforcement is disabled via the escape hatch (hatch can only DISABLE).
  it('escape hatch RUFLO_STRICT_AUTH=false → allow even with a denying scope', () => {
    process.env.RUFLO_STRICT_AUTH = 'false';
    expect(prop.checkToolCallOrInert('shell_exec', scope())).toEqual({
      allowed: true,
      reason: 'self-inert-disabled',
    });
  });

  it('escape hatch CLAUDE_FLOW_STRICT_AUTH=false is also honoured', () => {
    process.env.CLAUDE_FLOW_STRICT_AUTH = 'false';
    expect(prop.checkToolCallOrInert('shell_exec', scope())).toEqual({
      allowed: true,
      reason: 'self-inert-disabled',
    });
  ...
  // ON PATH: a real scope is present and enforcement is on → denial happens.
  it('real scope present, no hatch → ON: in-scope tool allowed', () => {
    expect(prop.checkToolCallOrInert('memory_store', scope())).toEqual({ allowed: true });
  });

  it('real scope present, no hatch → ON: out-of-scope tool DENIED', () => {
    expect(prop.checkToolCallOrInert('shell_exec', scope())).toEqual({
      allowed: false,
      reason: 'tool-not-in-scope',
    });
