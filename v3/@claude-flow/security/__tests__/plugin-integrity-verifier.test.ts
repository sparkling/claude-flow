/**
 * Tests for PluginIntegrityVerifier
 * (ADR-0324 / upstream 1c98cbee6 ADR-145 P1, ruvnet/ruflo#2254).
 *
 * Covers:
 ...
 *  - verify reports `signature-missing` for unsigned manifests (anchors present)
 ...
 *
 * AND the fork's ON+self-inert posture (feedback-no-dormant-off-by-default-flags):
 *  - SELF-INERT: empty trust-anchor set → `self-inert` (pass-through). This is
 *    the load-bearing divergence from upstream (which emits `unknown-signer`).
 *    Even a TAMPERED / unsigned manifest passes through when no anchors exist,
 *    proving zero behaviour change vs no-verification.
 *  - SELF-INERT: escape hatch (RUFLO_STRICT_PLUGINS=false) → pass-through even
 *    with anchors present and a bad signature.
 *  - ON PATH: once a real anchor is configured, a good signature passes and a
...
const EMPTY_ANCHORS: TrustAnchors = { version: 1, anchors: [] };
...
describe('findAnchor — trust resolution', () => {
  ...
  it('matches by exact key + matching exact scope', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@sparkleideas/memory' },
    ];
    expect(findAnchor(anchors, '@sparkleideas/memory', 'PK', NOW)).not.toBeNull();
  });

  it('matches by exact key + wildcard scope', () => {
    const anchors: TrustAnchor[] = [{ publicKey: 'PK', owner: 'o', scope: '@sparkleideas/*' }];
    expect(findAnchor(anchors, '@sparkleideas/anything', 'PK', NOW)).not.toBeNull();
  });

  it('rejects exact-key with out-of-scope plugin id', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@sparkleideas/*' },
    ];
...
// ─── self-inert (fork divergence) ────────────────────────────────────────

describe('PluginIntegrityVerifier — ON + self-inert (ADR-0324)', () => {
  afterEach(() => {
    delete process.env.RUFLO_STRICT_PLUGINS;
    delete process.env.CLAUDE_FLOW_STRICT_PLUGINS;
  });

  // SELF-INERT precondition: NO trust anchors → pass-through, NOT unknown-signer.
  it('empty trust anchors → self-inert pass-through for a valid-looking manifest', async () => {
    const v = new PluginIntegrityVerifier({ trustAnchors: EMPTY_ANCHORS });
    ...
    expect(r.status).toBe('self-inert');
    expect(r.pluginId).toBe('p');
  });

  // Zero behaviour change: even an unsigned / tampered manifest passes through
  // when no anchors exist — identical to no-verification.
  it('empty trust anchors → self-inert even for an unsigned manifest', async () => {
    const v = new PluginIntegrityVerifier({ trustAnchors: EMPTY_ANCHORS });
    const r = await v.verify({
      manifest: { id: 'unsigned', version: '0.0.0' },
      manifestHash: '',
      signature: '',
      publicKey: '',
    });
  ...
  // SELF-INERT escape hatch: anchors present + bad signature, but disabled.
  it('escape hatch RUFLO_STRICT_PLUGINS=false → self-inert even with anchors + bad sig', async () => {
    process.env.RUFLO_STRICT_PLUGINS = 'false';
    const v = new PluginIntegrityVerifier({ trustAnchors: anchorsFor('PK') });
    const r = await v.verify({
      manifest: { id: 'p', version: '1.0.0' },
      manifestHash: 'abc',
      signature: '', // would be signature-missing if enforced
      publicKey: 'PK',
    });
    expect(r.status).toBe('self-inert');
  });
...
// ─── verify (ON path — anchors present) ──────────────────────────────────

describe('PluginIntegrityVerifier.verify — ON path (anchors present)', () => {
  const skipMsg = '@noble/ed25519 unavailable — skipping signature round-trip tests';

  it('returns signature-missing when signature is empty (anchors present)', async () => {
    const v = new PluginIntegrityVerifier({ trustAnchors: anchorsFor('PK') });
    const r = await v.verify({
      manifest: { id: 'p', version: '1.0.0' },
      manifestHash: 'abc',
